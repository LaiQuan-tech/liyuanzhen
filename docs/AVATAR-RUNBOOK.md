...
### 0-1a　🔴 網路上找到的照片與影片，兩條路都走不通（2026-08-18 定案）

這個方向會反覆被提起，所以把結論寫死在這裡。

**技術上就過不了：ElevenLabs PVC 有強制的本人即時聲紋驗證。**
官方流程第 5 步「Verify your voice」——本人要用**相近的器材**現場唸驗證句，
過聲紋比對才會開始訓練，失敗要等 24 小時。這是他們的反濫用設計，
**用找到的影片做不出 PVC**，不是難做，是流程走不到那一步。

LiveAvatar 那邊也一樣：影片 avatar 要**同意錄影**（本人對 webcam 即時錄、
唸平台給的驗證碼）。唯一沒有這道關卡的是照片 avatar，但那條路的代價見階段 3-1。

**素材規格也差得遠。** 已檢核過的實例：

| 素材 | 實際規格 | 需要 | 差距 |
|---|---|---|---|
| 一張網路採訪照 | 515×388、20KB，臉部區域約 140×170px | 高解析、正面、素背景 | 側身、戴眼鏡、實景背景，解析度差一個量級 |
| 一段 FB 影片 | 選舉造勢短片 | 30 分鐘以上、單一講者、無背景音 | 長度差兩個量級 |

**著作權是獨立的第三道關。** 見附錄 B 第六項：照片與影片的著作權人通常
不是被拍的人。老師同意我們用她的臉，不涵蓋某位攝影師或某個粉專的作品。

#### ⚠️ 政治素材要特別避開

已查證的例子：某段 FB 影片是 2016 年（民國 105 年）立委選舉的造勢內容，
老師為特定候選人站台背書，貼文本身還附政治獻金帳號。

這個站是**財團法人婦女權益促進發展基金會**委製的，而 `lib/persona-prompt.ts`
與 answer-guard 明文拒答政黨傾向類問題（煙霧測試的對抗題就有「你支持哪一個政黨？」）。
**用她的黨派背書音檔去克隆聲音，跟這個站自己的設計直接衝突**，
而且一旦被發現，爭議不會停在技術層。

> 結論：素材一律**自己拍、自己錄**。這同時解決規格、驗證、著作權、政治風險
> 四件事，而它的成本就是階段 2 那一趟。

#### 順帶查到一個影響排程的事實

那則 2016 年的貼文提到老師已退休、回花蓮定居、當時身體有狀況要面對。

**拍攝地點要按花蓮規劃**，不是台北。這會影響器材運送、燈光、場地與當天的
時間安排——階段 2-2 那六項要在同一場做完，場地不能臨時找。
...
- [ ] 場地：安靜房間、深色乾淨背景、人距牆 1.5 公尺。⚠️ **在花蓮**，不是台北（老師已退休定居花蓮），器材與燈光要規劃運送
...
### 3-1　LiveAvatar 客製 avatar　👤 你送件 → 等約 24 小時

> 完成判準：拿到 `LIVEAVATAR_AVATAR_ID`。

#### ⚠️ 影片 avatar 對照片 avatar：這是三個決定綁在一起，不是一個

LiveAvatar 兩條路都有，看起來只差「素材好不好取得」，實際上照片那條會
連帶砍掉兩件我們已經規劃好的東西：
...
| 素材 | 2 分鐘影片 | 單張照片 |
| 同意錄影 | 要 | **不要** |
| **她的聲音克隆** | **自動附帶** | **完全沒有**——要另外指定聲音 |
| 神情與體態 | 從影片學到的是她的 | 由模型從照片**生成**，不是她的 |
| 待機 loop（省 30–50% 費用） | 同場多錄一分鐘就有 | **沒有素材可拍** |

**照片那條路的真實代價：**

1. **聲音回到原點。** 內建克隆是「建影片 avatar 時附帶生成」的。走照片就沒有
   這個附帶品，聲音只剩 ElevenLabs PVC（要 30 分鐘以上素材）或現成語音庫
   （只有籠統的 `zh`，不是她）。階段 0-1b 那張比較表的第一列直接消失。
2. **神情不是她的。** 官方寫得很含蓄——照片 avatar 的表情與體態是
   「Generated from the image」。對一位有高知名度、大量影像流傳在外的公眾人物，
   認識她的人看得出來哪裡不對，而那正是這個專案最怕的失敗模式。
3. **待機 loop 拍不到。** 那是目前成本結構裡最有效的一根槓桿。

⚠️ **還有一件要特別小心的事：照片 avatar 官方「不要求」同意錄影。**
那是平台的技術門檻，**不是我們的法律門檻**。平台沒有攔，不等於我們有授權——
授權要求見附錄 B，其中既有照片還多一關攝影師的著作權。

#### 現成照片當素材的技術檢核（官方四條）

官方對照片 avatar 的要求：高解析、光線充足、**正面對鏡頭**、
臉部清楚**不被配件或陰影遮擋**、背景**單純或素色**。

一般的採訪照／編輯照通常會踩到三條：人物側身或望向鏡頭外、戴眼鏡、
背景是實景（招牌、門框、街景）。挑素材時要拿這四條逐條對，
「這張照片很好看」跟「這張照片適合建模」是兩回事。

### 3-2　聲音克隆　🤖 我

⚠️ Azure 那條已取消（用途不合格，見階段 0-1）。現況：

- **走影片 avatar** → 內建克隆自動生成，這一步幾乎不用做
- **走 ElevenLabs PVC** → 用階段 2-2 第 3 項那批錄音，需先湊到 30 分鐘以上
...
並提早 2 秒自己收手，才有機會顯示「這段連線結束了，點一下按鈕就可以重新開始」。
...
客戶要的不是文字聊天框加小頭像，是「一張大臉、對著它說話、她直接回答」
...
> 🔴 **2026-09-03：按住說話已經整套改成「點一下切換」，這一段的 `setPointerCapture`
> 已經從程式碼裡移除。** 保留這段紀錄是因為它解釋了為什麼會有那道補丁。
>
> 改的理由三個：①手滑——手指移開按鈕範圍一點點就送出半句話，而
> `setPointerCapture` 只能減輕不能消除（它自己在 pointerId 已非作用中指標時
> 會丟 `NotFoundError`，原本還害過按鈕靜默失效）；②**每個新訪客的第一次都
> 注定失敗**——按住會跳麥克風權限詢問，要按「允許」就必須先放開按鈕，
> `pointerup` 一觸發就被判成 aborted；③**鍵盤完全按不到那顆按鈕**，
> 因為 Enter/Space 產生的是 `click`，不會有 pointer 事件。
>
> 現在只綁 `onClick`，判斷抽到 `lib/live/state.ts` 的 `talkButtonAction()`
> （有窮舉測試）。⚠️ 不要「順手」把 pointer 事件加回來當備援——
> 觸控上 click 本來就由 pointer 事件合成，兩套一起綁會讓每一下變成兩次。
>
> 錄音上限同時從 30 秒放寬到 45 秒（按住式有人的手當上限，切換式沒有），
> 連帶要動的三個地方寫在 `MAX_RECORDING_SECONDS` 的註解裡。
...
> ### 📌 本節已於 2026-08-19 執行完畢——但實際做的事比下表大一圈
>
> 下面這張 11 項的表是為「**換上真臉**」寫的：假設授權簽妥、要把揭露文案從
> 「我沒有她的臉」改成「臉是她的」。而 2026-08-19 實際執行的是
> 「**提案展示版轉正式站**」——那是兩件不同的事。換臉只動倫理文案，
> 轉正式站還要把整個網站從「給客戶看的 demo」變成「給陌生人看的對外網站」。
>
> 表格漏掉的部分（全部是實際踩到才發現的）：
>
> - **每一頁 `<title>` 裡的「提案展示版」** — 表格只查了頁面內文與常數，
>   沒查 `metadata`。分頁標籤上明晃晃寫著展示版，沒有人查
> - **頁首常駐的 `DEMO_BADGE` 膠囊** — 每一頁最顯眼的位置寫著「提案展示版」，已整個刪除
> - **`/events` 整頁** — 內文寫「線上報名功能已備妥…目前為提案展示版，尚未開放填寫」，
>   而且那還是空話：沒有表單元件也沒有 API route。改成照實寫「目前沒有已排定的場次」
> - **SEO 全套** — `app/sitemap.ts`、`metadataBase`、OG／Twitter card、`app/icon.svg`
>   當時通通不存在。⚠️ **正式站不等於開放收錄**：noindex 全部保留，
>   理由見下方「不要改的三樣」
> - **兩個當時就存在的 404 死連結** — `/timeline` 與 `/book`，導覽與內文都指過去，
>   從展示版時期就一直壞著，任何一份檢查表都沒抓到
>
> **教訓：「解除護欄」的檢查表不能拿來當「上線」的檢查表。** 前者只掃倫理相關的字串，
> 後者要掃的是「所有對訪客身分的假設」——demo 是給客戶看的，正式站是給陌生人看的。
>
> 下表保留原樣不刪：日後若再動一次授權範圍（例如加拍新素材、擴大用途），
> 這 11 個點仍然是要一起檢查的清單。
...
### ⚠️ 第六項：既有照片還有**攝影師**那一關（2026-08-18 補）

如果拿現成的採訪照／宣傳照當素材，需要的是**兩份**授權，不是一份：

| 誰 | 擁有什麼 | 沒拿到會怎樣 |
|---|---|---|
| 李元貞老師 | 肖像權 | 侵害人格權 |
| **拍照的攝影師／媒體** | **照片本身的著作權** | **侵害著作財產權** |

這兩件事完全獨立。老師同意我們用她的臉，不代表我們有權使用某位攝影師拍的
那張照片去訓練模型。編輯照、媒體採訪照、活動紀錄照幾乎都有明確的著作權人，
而且通常不是被拍的人。

**這就是為什麼「我們自己拍那 2 分鐘」在法律上比「找一張現成好照片」乾淨得多**
——自己拍，著作權從一開始就在我們手上，只要處理老師這一份授權。

> 完成判準：若最終使用任何既有影像，該影像的著作權人書面同意「用於 AI 模型
> 訓練與合成影像生成」——一般的「授權轉載」不涵蓋這個用途，要另外寫明。
