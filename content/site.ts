...
export const liveCopy = {
   ...
   * 第一次來訪必踩的洞，不是誤觸。
   *
   * ⚠️ 按住說話 → 瀏覽器跳出麥克風權限詢問 → 想按「允許」就**必須先放開按鈕**
   * → pointerup 觸發 → 這一輪被判定成 aborted。也就是說每一個新訪客的
   * 第一次嘗試都注定失敗，而舊版給他的是「按住不放，講完再放開。」——
   * 指著錯的方向，他只會一直重複同一個注定失敗的動作。
   *
   * 用 navigator.permissions 確認權限還停在 prompt 才顯示這句，
   * 真正的誤觸仍然走 tooShort。
   */
  micNeedsAllow: "請先在瀏覽器的提示裡按「允許」使用麥克風，然後再按一次按住說話。",
  ...
  /**
   * 跟 micDead 同一件事，差別在**時機**：這一句是在訪客還在講話的當下就出現的。
   *
   * ⚠️ 它不是音量判斷，所以不受 recordingHint 那條「不要寫判斷句」的限制。
   * 觸發條件是接上音訊圖、重接一次之後仍然**零塊**音訊——那是管線死掉的事實，
   * 不是「講得不夠大聲」的猜測。讓他當下就停下來，比講完三秒才發現好。
   */
  micSilentLive: "麥克風沒有送出聲音。請重新整理這一頁再試一次。",
