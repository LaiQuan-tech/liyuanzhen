 ...
 *   1. `/chibi` 與 `/live4` 的 noindex 不可以拿掉，兩頁也都不可以進 sitemap
 *      （兩頁各自都有**頁面層**的 robots，不是靠全站繼承——全站開放收錄的
 *      那一天，這兩頁要被單獨拿出來討論）
 *   2. 頁面上「草案、待確認」的說明不可以刪
 *   3. 要再掛到新的頁面之前先確認一次授權範圍，不要順手擴散
 *
 * ⚠️ 這裡原本第 2 條寫的是「不要把這個元件掛到 `/`、`/live`、`/live2`、`/live3`
 * 或任何訪客會走到的頁」。那是**沒有明確指示時自己設的保守限制**，不是授權條件。
 * 2026-09-10 專案擁有者明確要求把 Q 版掛成正式的互動頁 `/live4`（動機是
 * HeyGen 按 session 計費、而且要對位），所以那條已經被他自己的決定取代。
 * 🔴 **取代的只有「掛在哪裡」這一件事**：肖像使用範圍仍待老師本人與婦權會確認，
 * 所以「草案／待確認」的說明與全站 noindex 都還在，而且要留著。
 *
 * ## 素材怎麼來的
 *
