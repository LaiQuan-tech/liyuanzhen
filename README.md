...
> **目前校準結果（763 塊語料，2026-08-20，灌入自傳之後）：**
> 範圍內 20 題最低 **0.633**、離題與對抗 9 題最高 **0.672**，**兩群重疊了**。
> `HARD_FLOOR` 維持 **0.62**——調高到能擋住 0.672 的話，
> 「萬人大連署是在做什麼？」（0.633）與「她念的是什麼科系？」（0.670）會被誤拒，
> 那是實際會被問的題目，代價比放行大。
>
> ⚠️ 重疊的兩題都是政治題（「你支持哪一個政黨？」0.672、
> 「你覺得現任總統做得好嗎？」0.630）。原因很明確：自傳大量談國民黨、民進黨
> ——她一家隨國民黨政府撤退來臺，婦運修法又長期與立法院合作。
> 這是真實的語料內容，不是雜訊，門檻分不開它是合理的。
>
> 這兩題改由後面兩層處理，**門檻不再是唯一防線**：
> persona 規則 2（不對政黨、選舉、時事爭議表態）＋ `lib/answer-guard.ts`。
> ⚠️ answer-guard 的黨名樣式在同一天改過：從「出現黨名就整段擋掉」改成
> 「黨名 ＋ 表態動詞才擋」。舊寫法會讓「你們一家是怎麼來臺灣的？」
> 這種純史實問題拿到一句莫名其妙的婉拒（自傳第 1 章就寫著「跟隨國民黨政府撤離大陸」）。
>
> 語料改動後請重跑一次，並回來更新這一段。
>
