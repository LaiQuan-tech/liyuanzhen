...
/**
 * 把訪客在 /live 錄下來的音訊轉成文字。/live 的第一站。
 *
 ...
 * 45 秒約 135KB；正式站實測 Chrome 錄 4 秒是 7KB（比要求的還低）。
 *
 * ⚠️ 但**不可以**照 Chrome 的數字抓。`audioBitsPerSecond` 是建議不是保證，
 * Safari 的 AAC 預設在 64~128kbps，45 秒就是 360~720KB。抓 250KB 的話，
 * 一個用 iPhone 問了長問題的訪客會拿到 413——而那在畫面上跟壞掉沒有分別。
 * 1MB 讓最壞情況（128kbps × 45 秒 ≈ 720KB）也進得來，還留一點餘裕。
 *
 * 🔴 2026-09-03 從 600KB 調到 1MB，因為客戶端的錄音上限從 30 秒改成 45 秒
 * （互動改成「點一下切換」，見 lib/live/recorder.ts 的 MAX_RECORDING_SECONDS）。
 * 這兩個數字是綁在一起的，只改一邊 iPhone 上的長問題就會 413。
 *
